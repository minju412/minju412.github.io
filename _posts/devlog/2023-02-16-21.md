---
title:  "[AWS] Jenkins를 통한 Docker Compose + mysql+ Spring Boot 애플리케이션 빌드 및 자동 배포 찐찐찐찐찐 "
...
젠킨스가 webhook을 통해 가져와서 자동으로 build 하고, `Execute shell`에 적은 `docker-compose up -d` 커맨드를 통해 배포까지 진행한다.
