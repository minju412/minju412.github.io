...
$ docker logs mysql
```
![스크린샷 2022-10-20 오전 10 22 58](https://user-images.githubusercontent.com/59405576/196834125-52959e45-b995-402c-9f50-628d4c8ac58d.png)<br><br>
...
시도 2. 컨테이너 실행 시 로그 저장 명령 추가<br>
아래 커맨드로 mysql 컨테이너를 다시 기동한다.
```bash
$ docker run --platform linux/x86_64 -d -p 3306:3306 --network naem-network \
-e "logging.file=/api-logs/mysql.log" \
--name mysql \
ln8847/naem_mysql:1.0
```
로그를 확인해보려 했지만, 생성되지 않은 것 같다..<br>
`-e "logging.file=/api-logs/mysql.log" \` 옵션은 빼고 실행해도 될 듯 하다.<br><br>
